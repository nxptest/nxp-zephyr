...
K_THREAD_STACK_DEFINE(aws_stack, 4096);
struct k_thread aws_thread;
...
void wait_for_network_ip(void){
    struct net_if *iface = net_if_get_default();

    while (!net_if_is_up(iface) ||
           !net_if_ipv4_get_global_addr(iface, NET_ADDR_ANY)) {
        printk("Waiting for DHCP lease...\n");
        k_sleep(K_MSEC(500));
    }
    printk("Network is ready, IP acquired!\n");
}
                    ...
                    uint32_t event, struct net_if *iface)
{
    if (event == NET_EVENT_IPV4_ADDR_ADD) {
        printk("DHCP lease acquired: starting AWS...\n");
        NetworkUp = 1;
    }
}
...
void sync_time(void)
{
    struct sntp_time sntp_time;int err;

    /* Query an NTP server with 2s timeout */
	err = sntp_simple("pool.ntp.org", 2000, &sntp_time);
    if (err) {
        printk("SNTP request failed: %d\n", err);
        return;
    }

    /* Convert SNTP result to timespec */struct timespec ts = {
        .tv_sec = sntp_time.seconds,
        .tv_nsec = ((uint64_t)sntp_time.fraction * NSEC_PER_SEC) >> 32,
    };

    /* Set Zephyr system clock */if (clock_settime(CLOCK_REALTIME, &ts) == 0) {
        printk("System time set: %lld\n", (long long)ts.tv_sec);
    } else {
        printk("Failed to set system time\n");
    }
}

void aws_thread_entry(void *, void *, void *)
{
   	//wait_for_network_ip();
	while(!NetworkUp)
	{
...
void start_aws_thread(void)
{
    k_thread_create(&aws_thread, aws_stack, K_THREAD_STACK_SIZEOF(aws_stack),
                    aws_thread_entry, NULL, NULL, NULL,
